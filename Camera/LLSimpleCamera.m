...
- (void)captureOutput:(AVCaptureFileOutput *)captureOutput didFinishRecordingToOutputFileAtURL:(NSURL *)outputFileURL fromConnections:(NSArray *)connections error:(NSError *)error
{
    ...
    NSString *recordedPath = [outputFileURL path];

    if (UIVideoAtPathIsCompatibleWithSavedPhotosAlbum(recordedPath)) {
        UISaveVideoAtPathToSavedPhotosAlbum(recordedPath, self, @selector(video:didFinishSavingWithError:contextInfo:), nil);
    } else {
        NSLog(@"couldn't save to %@", recordedPath);
    }
...
- (BOOL)updateFlashMode:(LLCameraFlash)cameraFlash
{
    ...
    return [[self.photoOutput supportedFlashModes] containsObject:@(flashMode)];
}
...
{
    NSArray<AVCaptureDeviceType> *types = [NSArray arrayWithObjects:AVCaptureDeviceTypeBuiltInMicrophone, AVCaptureDeviceTypeBuiltInWideAngleCamera, nil];
    AVCaptureDeviceDiscoverySession *session = [AVCaptureDeviceDiscoverySession discoverySessionWithDeviceTypes:types mediaType:nil position:position];
